    ...
    <div className="min-h-screen bg-neo-bg py-10 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <nav className="flex text-sm font-body font-bold text-gray-600 mb-8 items-center gap-2 flex-wrap">
          <Link href="/" className="hover:text-neo-accent hover:underline">Ana Sayfa</Link>
          <span>/</span>
          <Link href={`/platform/${problem.platform}`} className="hover:text-neo-accent hover:underline uppercase">
            {problem.platform}
          </Link>
          <span>/</span>
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 mt-12">
          {prevProblem ? (
            <Link href={`/problem/${prevProblem.id}`} className="neo-btn bg-neo-white flex flex-col items-start w-full sm:w-1/2 group">
              <span className="text-sm text-gray-500 font-body">← Önceki Soru</span>
          ...
          {nextProblem ? (
            <Link href={`/problem/${nextProblem.id}`} className="neo-btn bg-neo-white flex flex-col items-end w-full sm:w-1/2 group text-right">
              <span className="text-sm text-gray-500 font-body">Sonraki Soru →</span>
