...
const platformMeta = {
  leetcode: {
    ...
    icon: "🏆",
    description:
      "Mülakat hazırlığı, Veri Yapıları ve Algoritmalar (DSA) temelleri.",
    color: "bg-neo-orange",
...
export default async function PlatformPage({ params }) {
    ...
    return (
      <div className="min-h-screen py-20 px-4 flex items-center justify-center">
        <div className="neo-card bg-neo-red text-white text-center p-12">
          ...
          <a
            href="/"
            className="neo-btn bg-neo-white text-neo-black inline-block mt-4"
          >
            Ana Sayfaya Dön
          </a>
        </div>
  ...
  const problems = await getProblemsByPlatform(slug);

  return (
    <div className="min-h-screen bg-neo-white">
      {/* Platform Başlık */}
      <section
        className={`${meta.color} ${meta.textClass} border-b-3 border-neo-black py-16 px-4`}
      >
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="text-center md:text-left">
            ...
            <p className="text-xl font-body font-medium opacity-90 max-w-2xl">
              {meta.description}
            </p>
          </div>
        </div>
