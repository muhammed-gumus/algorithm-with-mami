...
export async function GET(request, { params }) {
  try {
    ...
    if (problem) {
      ...
      return Response.json({ success: false, error: 'Soru bulunamadı' }, { status: 404 });
    }
  } catch (error) {
...
export async function PUT(request, { params }) {
  try {
    ...
    const updatedProblem = updateProblem(id, body);

    if (updatedProblem) {
      return Response.json({ success: true, problem: updatedProblem });
    } else {
      return Response.json({ success: false, error: 'Soru bulunamadı veya güncellenemedi' }, { status: 404 });
    }
  } catch (error) {
    return Response.json({ success: false, error: error.message }, { status: 500 });
  }
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (success) {
      ...
      return Response.json({ success: false, error: 'Soru bulunamadı veya silinemedi' }, { status: 404 });
    }
  } catch (error) {
    return Response.json({ success: false, error: error.message }, { status: 500 });
  }
